...
* 只要事先對 $$S_1$$ 中的點依照 $$(x, y)$$ 座標排序，就能在 $$O(\lvert T_1\rvert)$$ 時間內以 [Andrew's 演算法](https://en.wikibooks.org/wiki/Algorithm_Implementation/Geometry/Convex_hull/Monotone_chain)求出 $$\operatorname{Conv}(T_1)$$，總時間複雜度為 $$O(nm2^n)$$。
